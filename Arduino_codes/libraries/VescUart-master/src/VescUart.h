...
class VescUart
{
	...
	struct dataPackage {
		float tempFet;        // Added by Salam to get Temperature from VESC Mosfet 23/7/2020
		float tempMotor;
	...
	public:
		...
		/**
		 * @brief      Sends a command to VESC and stores the returned data from ID 
		 * @param	   motorId - VESC CAN Bus ID // Added By Salam
		 * @return     True if successfull otherwise false
		 */
		bool getVescValues(uint8_t motorId);
