...
int VescUart::receiveUartMessage(uint8_t * payloadReceived) {
	...
	uint32_t timeout = millis() + 200; // Defining the timestamp for timeout (100ms before timeout)
...
bool VescUart::processReadPacket(uint8_t * message) {
	...
	switch (packetId){
		case COMM_GET_VALUES: // Structure defined here: https://github.com/vedderb/bldc/blob/43c3bbaf91f5052a35b75c2ff17b5fe99fad94d1/commands.c#L164
			...
			data.tempFet 			= buffer_get_float16(message, 10.0, &ind); // Added by Salam to get Temperature from VESC Mosfet 23/7/2020
...
bool VescUart::getVescValues( uint8_t motorId) { // Added by Salam
	uint8_t payload[256];
 
	uint8_t command[3] = {COMM_FORWARD_CAN ,motorId, COMM_GET_VALUES };	
...
void VescUart::setCurrent(float current, int motorId) {
	...
	payload[index++] = COMM_SET_CURRENT;
	buffer_append_int32(payload, (int32_t)(current * 1000), &index);
	//buffer_append_float32(payload,  current,  1000,  &index) 
